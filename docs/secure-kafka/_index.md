...
To securely connect to a listener with mTLS enabled, we need to mount the `my-cluster-cluster-ca-cert` secret onto our Flink pod _and_ pass the contents of the certificate and key from the `my-user` secret generated by Strimzi to our Flink SQL statements.
By doing this, we can instruct Flink to trust the Kafka cluster's TLS certificate _and_ to send on our own credentials.
